...
public class PersistenciaTiquetesJson implements IPersistenciaTiquetes {
	...
	/**
	 * Carga la información de los clientes y tiquetes vendidos por la aerolínea, y
	 * actualiza la estructura de objetos que se encuentra dentro de la aerolínea
	 * 
	 * @param archivo   La ruta al archivo que contiene la información que se va a
	 *                  cargar
	 * @param aerolinea La aerolínea dentro de la cual debe almacenarse la
	 *                  información
	 * @throws IOException                       Se lanza esta excepción si hay
	 *                                           problemas leyendo el archivo
	 * @throws InformacionInconsistenteException Se lanza esta excepción si hay
	 *                                           información inconsistente dentro
	 *                                           del archivo, o entre el archivo y
	 *                                           el estado de la aerolínea
	 */
	@Override
	...
	/**
	 * Salva en un archivo toda la información sobre los clientes y los tiquetes
	 * vendidos por la aerolínea
	 * 
	 * @param archivo   La ruta al archivo donde debe quedar almacenada la
	 *                  información
	 * @param aerolinea La aerolínea que tiene la información que se quiere
	 *                  almacenar
	 * @throws IOException Se lanza esta excepción si hay problemas escribiendo el
	 *                     archivo
	 */
	@Override
	 ...
	 * @throws ClienteRepetidoException Lanza esta excepción si alguno de los
	 *                                  clientes que se van a cargar tiene el mismo
	 *                                  identificador que otro cliente
	 */
	private void cargarClientes(Aerolinea aerolinea, JSONArray jClientes) throws ClienteRepetidoException {
		...
		for (int i = 0; i < numClientes; i++) {
			...
			// En las siguientes líneas se utilizan dos estrategias para implementar la
			// carga de objetos: en la primera estrategia, la carga de los objetos
			// lo hace alguien externo al objeto que se carga; en la segunda estrategia, los
			// objetos saben cargarse.
			// En general es una mala idea mezclar las dos estrategias: acá lo hacemos para
			// ilustrar las dos posibilidades y mostrar las ventajas y desventajas de cada
			// una.
			// Lo que sí es recomendable es seleccionar una estrategia y usarla
			// consistentemente para cargar y salvar.
			if (ClienteNatural.NATURAL.equals(tipoCliente)) {
				// 1. En esta estrategia, en ESTA clase se realiza todo lo que tiene que ver con
				// cargar objetos de la clase ClienteNatural
				// Al revisar el código de la clase ClienteNatural, no hay nada que tenga que
				// ver con cargar o salvar.
				// En este caso, la persistencia es una preocupación transversal de la que no se
				// ocupa la clase ClienteNatural
				String nombre = cliente.getString(NOMBRE_CLIENTE);
				nuevoCliente = new ClienteNatural(nombre);
			} else {
				// 2. En esta estrategia, en la clase ClienteCorporativo se realiza una parte de
				// lo que tiene que ver con cargar objetos de la clase ClienteCorporativo.
				// La clase ClienteCorporativo tiene un método para cargar y otro para salvar.
				// En este caso, la persistencia es una preocupación de la cual se ocupa la
	...
	/**
	 * Salva la información de los clientes de la aerolínea dentro del objeto json
	 ...
	 * @param jobject   El objeto JSON donde debe quedar la información de los
	 *                  clientes
	 */
	private void salvarClientes(Aerolinea aerolinea, JSONObject jobject) {
		...
		for (Cliente cliente : aerolinea.getClientes()) {
			...
			// Para los clientes naturales, esta clase extrae la información de los objetos
			// y la organiza para que luego sea salvada.
			// Para los clientes corporativos, la clase ClienteCorporativo hace todo lo que
			// está en sus manos para persistir un cliente
	 ...
	 * @throws InformacionInconsistenteTiqueteException Lanza esta excepción si la
	 *                                                  información de alguno de los
	 *                                                  tiquetes no es consistente
	 *                                                  con el resto de elementos de
	 *                                                  la aerolínea (ej. es un
	 *                                                  tiquete para un vuelo que no
	 *                                                  existe, o fue comprado por
	 *                                                  un cliente que no existe,
	 *                                                  etc.)
	 */
	private void cargarTiquetes(Aerolinea aerolinea, JSONArray jTiquetes)
	...
	/**
	 * Salva la información de los tiquetes de la aerolínea dentro del objeto json
	 * que se recibe por parámetro.
	 * 
